...
Usage:
    uv run --project services/catalog/scripts fetch_series_seed.py \
        "Stranger Things" \
...
def emit_sql(show_title: str, slug: str, episodes: list[EpisodeRow], output_path: str) -> None:
    # The uuid5 namespace key stays "show:" so regenerating reproduces the
    # series UUID already committed in the seed migration; changing it would
    # churn every id. The emitted SQL uses the "series" table and columns.
    series_id = uuid.uuid5(NAMESPACE, f"show:{slug}")
    ...
    lines.append("-- Auto-generated by services/catalog/scripts/fetch_series_seed.py")
    lines.append(f"-- Generated at: {now}")
    lines.append("-- Edit by re-running the script, not by hand.")
    lines.append("")
    lines.append(
        f"INSERT INTO series (id, title, seasons_count, episodes_count) VALUES "
        f"('{series_id}', $${show_title}$$, {seasons_count}, {episodes_count});"
    )
    lines.append("")
    for e in episodes:
        ...
        lines.append(
            "INSERT INTO episode "
            "(id, series_id, season, episode_number, episode_index, title, summary) VALUES ("
            f"'{ep_id}', '{series_id}', {e.season}, {e.episode_number}, "
